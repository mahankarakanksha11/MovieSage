...
def get_movie_info(imdb_link):
    ...
    movie_director = movie_descr[0].strip() if len(movie_descr) > 0 else ""
    movie_cast = str(movie_descr[1]).replace('With', 'Cast: ').strip() if len(movie_descr) > 1 else ""
